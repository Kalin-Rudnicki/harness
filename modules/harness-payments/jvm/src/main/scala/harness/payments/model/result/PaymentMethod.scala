...
object PaymentMethod {
  ...
  private implicit class StripePaymentMethodOps(paymentMethod: StripePaymentMethod) {
    ...
    def requireField[A](fieldName: String)(f: StripePaymentMethod => A): IO[PaymentError.InternalDefect, A] =
      ZIO.getOrFailWith(PaymentError.InternalDefect(s"StripePaymentMethod is missing required field '$fieldName'"))(Option(f(paymentMethod)))
