...
import harness.archive.api.db.queries.{Session, User}
import harness.archive.model.user.{Login, SignUp}
  ...
  val routes: Route[JDBCConnection & Transaction] =
    "user" /: Route.oneOf(
      ...
      (HttpMethod.POST / "login").implement { _ =>
        Transaction.inTransaction {
          for {
            body <- HttpRequest.jsonBody[Login]
            ...
            _ <- Session.insert(session).single
      ...
      (HttpMethod.POST / "sign-up").implement { _ =>
        Transaction.inTransaction {
          for {
            body <- HttpRequest.jsonBody[SignUp]
