...
object Url {
  ...
  val fromWindowURL: Task[Url] =
    for {
      pathname <- ZIO.attempt(window.location.pathname)
      search <- ZIO.attempt(window.location.search)

      path = pathname.split("/").toList.filter(_.nonEmpty)
      params <- ZIO.attempt { new URLSearchParams(search).toList.map { t => (t._1, t._2) }.toMap }
