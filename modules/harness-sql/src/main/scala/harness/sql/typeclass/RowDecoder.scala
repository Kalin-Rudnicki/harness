...
trait RowDecoder[T] { self =>
  ...
  lazy val width: Int
  lazy val classes: IArray[Option[Class[?]]]
  ...
  final def map[T2](f: T => T2): RowDecoder[T2] =
    new RowDecoder[T2] {
      ...
      override lazy val classes: IArray[Option[Class[?]]] = self.classes
  ...
  final def ~[T2](other: RowDecoder[T2])(implicit z: Zip[T, T2]): RowDecoder[z.Out] =
    new RowDecoder[z.Out] {
      ...
      override lazy val classes: IArray[Option[Class[?]]] = self.classes ++ other.classes
...
object RowDecoder {
  ...
  def fromColDecoder[T](cd: ColDecoder[T]): RowDecoder[T] =
    new RowDecoder[T] {
      ...
      override lazy val classes: IArray[Option[Class[?]]] = IArray(cd.klass)
    }
  ...
  def forTable[T[_[_]] <: Table](t: T[Col])(using inst: => K11.ProductGeneric[T]): RowDecoder[T[Id]] =
    new RowDecoder[T[Id]] {
      ...
      override lazy val classes: IArray[Option[Class[?]]] = decoders.map(_.klass)
    }
