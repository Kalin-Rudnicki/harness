...
trait ColDecoder[T] { self =>
  ...
  final def map[T2](f: T => T2): ColDecoder[T2] =
    new ColDecoder[T2] {
      ...
      override val klass: Option[Class[_]] = self.klass
    }
...
object ColDecoder {
  ...
  private def unsafeDecode[T: JsonDecoder](k: Option[Class[_]] = None)(pf: PartialFunction[Object, T]): ColDecoder[T] =
    new ColDecoder[T] {
      override def decodeColumn(c: Object): EitherNel[String, T] = pf(c).asRight
      override val jsonDecoder: JsonDecoder[T] = JsonDecoder[T]
      override val klass: Option[Class[_]] = k
    }

  private def unsafeDecodeE[T: JsonDecoder](k: Option[Class[_]] = None)(pf: PartialFunction[Object, EitherNel[String, T]]): ColDecoder[T] =
    new ColDecoder[T] {
