...
object MigrationRunner {
  ...
  def runMigrations(migrations: PlannedMigrations): ZIO[HarnessEnv & JDBCConnection, MigrationError, Unit] =
    doMigrations(migrations.migrations.toList, true)

  def runMigrationsFromPool(migrations: PlannedMigrations): ZIO[HarnessEnv & JDBCConnectionPool, MigrationError, Unit] =
    ZIO.scoped {
      MigrationRunner.runMigrations(migrations).provideSomeLayer[HarnessEnv & JDBCConnectionPool & Scope](JDBCConnection.poolLayer.mapError(MigrationError.ConnectionError(_)))
    }

  def rollbackTo(version: Version): ZIO[HarnessEnv & JDBCConnection, MigrationError, Unit] =
    for {
  ...
  def rollbackToFromPool(version: Version): ZIO[HarnessEnv & JDBCConnectionPool, MigrationError, Unit] =
    ZIO.scoped {
      MigrationRunner.rollbackTo(version).provideSomeLayer[HarnessEnv & JDBCConnectionPool & Scope](JDBCConnection.poolLayer.mapError(MigrationError.ConnectionError(_)))
    }

  def rollbackExecutable(dbLayer: ZLayer[HarnessEnv & Scope, ConnectionError, JDBCConnectionPool]): Executable =
    Executable
      .withParser { harness.cli.Parser.value[Version](harness.cli.LongName.unsafe("version")) }
      .withLayer { dbLayer.mapError(MigrationError.ConnectionError(_)) }
      .withEffect { version => MigrationRunner.rollbackToFromPool(version) }

  // =====|  |=====

  private def doMigrations(inMemoryMigrations: List[InMemoryMigration], persist: Boolean): ZIO[HarnessEnv & JDBCConnection, MigrationError, Unit] =
    for {
  ...
  private val getExisting: ZIO[HarnessEnv & JDBCConnection, MigrationError, List[Migration.Identity]] =
  ...
  private val createMigrationsTableIfDNE: ZIO[HarnessEnv & JDBCConnection, MigrationError, Unit] =
    getSchemas().list
  ...
  private def undoMigration(migration: Migration.Identity): ZIO[HarnessEnv & JDBCConnection, MigrationError, Unit] =
    Transaction
      .inTransaction {
        for {
  ...
  private implicit val errorMapper: ErrorMapper[QueryError, MigrationError] =
    MigrationError.QueryError(_)
